...
class LV:
    ...
    def sim(self, n_samples=1000, rng=np.random):
        ...
        for i in range(n_samples):
            ...
            xs[i, : self.n_sensors] = sim_model.predator[
                self.measure_time
            ] + self.noise_std * np.random.randn(self.n_sensors)
            xs[i, self.n_sensors :] = sim_model.prey[
                self.measure_time
            ] + self.noise_std * np.random.randn(self.n_sensors)
    ...
    def sim_joint(self, n_samples=1000, rng=np.random):
        ...
        for i in range(n_samples):
            ...
            xs[i, 0] = a
            xs[i, 1] = b
            xs[i, 2 : self.n_sensors + 2] = sim_model.predator[
                self.measure_time
            ] + self.noise_std * np.random.randn(self.n_sensors)
            xs[i, self.n_sensors + 2 :] = sim_model.prey[
                self.measure_time
            ] + self.noise_std * np.random.randn(self.n_sensors)
    ...
    def plot_many_sims(self, n_samples=1000):
        ...
        for i in range(n_samples):
            ...
            ax.plot(
                sim_model.time,
                predator + self.noise_std * np.random.randn(predator.shape[0]),
                "-b",
                label="Predator",
                linewidth=0.1,
                alpha=0.5,
            ...
            ax.plot(
                sim_model.time,
                prey + self.noise_std * np.random.randn(prey.shape[0]),
                "-r",
                label="Prey",
                linewidth=0.1,
                alpha=0.5,
...
class Lotka_Volterra:
    ...
    def integrate(self):
        ...
        for i in range(self.n - 1):
            ...
            self.prey[i + 1] = self.prey[i] + self.dt * self.prey[i] * (
                self.preygrow - self.predator[i] * self.preydie
            )
    ...
    def integrate_logistic(self):
        ...
        for i in range(self.n - 1):
            self.time[i + 1] = self.time[i] + self.dt
            self.predator[i + 1] = self.predator[i] + self.dt * self.predator[i] * (
                self.predgrow * self.prey[i] * (1.0 - self.predator[i] / self.predator_capacity)
                - self.preddie
            )
            self.prey[i + 1] = (
                self.prey[i]
                + self.dt * self.prey[i] * self.preygrow * (1.0 - self.prey[i] / self.prey_capacity)
                - self.prey[i] * self.predator[i] * self.preydie
            )
...
class PK:
    ...
    def sim(self, n_samples=1000, rng=np.random):
        ...
        for i in range(n_samples):
            ...
            xs[i] = self.D / V * ka / (ka - ke) * (
                np.exp(-ke * self.measure_time) - np.exp(-ka * self.measure_time)
            ) * (1 + self.sig_prop * rng.randn(self.n_sensors)) + self.sig_add * rng.randn(
                self.n_sensors
            )
    ...
    def plot_response(self, n_samples=100, rng=np.random):
        ...
        for i in range(n_samples):
            ...
            ke = np.exp(np.log(0.1) + np.sqrt(0.05) * rng.randn())
            V = np.exp(np.log(20.0) + np.sqrt(0.05) * rng.randn())
            y = self.D / V * ka / (ka - ke) * (np.exp(-ke * t) - np.exp(-ka * t)) * (
                1 + self.sig_prop * rng.randn(n)
            ) + self.sig_add * rng.randn(n)
            plt.plot(t, y)

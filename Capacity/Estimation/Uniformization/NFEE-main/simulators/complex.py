...
class n_d_beta:
    ...
    def mcmc_entropy(self, n_samples=1000):
        return self.x_dim * (
            np.log(spl.beta(self.a, self.b))
            - (self.a - 1) * spl.digamma(self.a)
            - (self.b - 1) * spl.digamma(self.b)
            + (self.a + self.b - 2) * spl.digamma(self.a + self.b)
        )
        # return -np.mean(np.log(self.density_fun(self.sim(n_samples))))
...
class n_d_quadratic:
    ...
    def density_fun(self, xs):
        ...
        dens = stats.multivariate_normal.pdf(
            xs[:, : self.sigma - 1], mean=np.zeros(self.sigma - 1), cov=np.eye(self.sigma - 1)
        ) * stats.norm.pdf(
            xs[:, -1] - np.sum(xs[:, : self.sigma - 1] ** 2, axis=1), scale=self.noise
        )
    ...
    def sim(self, n_samples=1000, rng=np.random):
        ...
        for j in range(self.n_2):
            ...
            for i in range(self.n_1 - 1):
                ...
                if i == 0:
                    new_sample = samples[:, 0][:, np.newaxis] ** 2 + np.sqrt(
                        0.5 / self.b[j, i]
                    ) * rng.randn(n_samples, 1)
                else:
                    new_sample = samples[:, -1][:, np.newaxis] ** 2 + np.sqrt(
                        0.5 / self.b[j, i]
                    ) * rng.randn(n_samples, 1)
    ...
    def density_fun(self, xs):
        ...
        for j in range(self.n_2):
            ...
            for i in range(self.n_1 - 1):
                ...
                if i == 0:
                    dens = dens * stats.norm.pdf(
                        xs[:, j * (self.n_1 - 1) + i + 1] - xs[:, 0] ** 2,
                        scale=np.sqrt(0.5 / self.b[j, i]),
                    )
                else:
                    dens = dens * stats.norm.pdf(
                        xs[:, j * (self.n_1 - 1) + i + 1] - xs[:, j * (self.n_1 - 1) + i] ** 2,
                        scale=np.sqrt(0.5 / self.b[j, i]),
                    )

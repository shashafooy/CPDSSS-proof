...
class mvn:
    ...
    def __init__(self, rho=0.8, dim_x=1):
        ...
        self.S = np.concatenate(
            (
                np.concatenate((self.sigma11, self.sigma12), axis=1),
                np.concatenate((self.sigma21, self.sigma22), axis=1),
            ),
            axis=0,
        )
...
class mvquad:
    ...
    def density_fun(self, xs):
        ...
        dens = stats.multivariate_normal.pdf(
            xs[:, : self.dim_x], mean=np.zeros(self.dim_x), cov=np.eye(self.dim_x)
        ) * stats.multivariate_normal.pdf(
            xs[:, self.dim_x :] - xs[:, : self.dim_x] ** 2,
            mean=np.zeros(self.dim_x),
            cov=self.rho**2 * np.eye(self.dim_x),
        )
    ...
    def mcmc_entropy(self, n_samples=1000):
        ...
        return 0.5 * np.log(
            np.linalg.det(2 * math.pi * np.exp(1) * np.eye(self.dim_x))
        ) + 0.5 * np.log(
            np.linalg.det(2 * math.pi * np.exp(1) * self.rho**2 * np.eye(self.dim_x))
        )
    ...
    def mi(self, n_samples=1000, rng=np.random):
        ...
        for i in range(n_samples):
            ...
            for j in range(n_samples):
                p_yx_sum = p_yx_sum + 1.0 / n_samples * stats.norm.pdf(
                    ys[i] - xs[j] ** 2, 0, self.rho
                )
...
class mvquad2:
    ...
    def density_fun(self, xs):
        ...
        dens = stats.multivariate_normal.pdf(
            xs[:, : self.dim_x], mean=np.ones(self.dim_x), cov=np.eye(self.dim_x)
        ) * stats.multivariate_normal.pdf(
            xs[:, self.dim_x :] - xs[:, : self.dim_x] ** 2,
            mean=np.zeros(self.dim_x),
            cov=self.rho**2 * np.eye(self.dim_x),

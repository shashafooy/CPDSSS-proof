...
for i in range(n_trials):
    ...
    for ns, num_nodes in enumerate(hidden_nodes):
        ...
        H_unif_KL[2 * i, ns], H_unif_KSG[2 * i, ns] = ent.knn_entropy(
            estimator, laplace_base, method=method
        )
        H_unif_KL[2 * i + 1, ns], H_unif_KSG[2 * i + 1, ns] = ent.knn_entropy(
            estimator, sim_laplace.sim(knn_samples), method=method
        )
        ...
        util.io.save(
            (hidden_nodes, H_unif_KL, H_unif_KSG, H_KL, H_KSG), os.path.join(path, filename)
        )

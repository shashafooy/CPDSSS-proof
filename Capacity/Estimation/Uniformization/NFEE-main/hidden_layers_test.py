...
for i in range(n_trials):
    ...
    for ni, n_layers in enumerate(layers):
        for nj, n_nodes in enumerate(nodes):
            ...
            misc.print_border(
                "Calculate H(x) laplace, Nodes={}, Layers={}, iter: {}".format(
                    n_nodes, n_layers, i + 1
                )
            )
            ...
            uniform, correction2 = estimator.uniform_correction(
                sim_laplace.sim(n_samples=knn_samples)
            )
            ...
            util.io.save(
                (layers, nodes, H_unif_KL, H_unif_KSG, H_KL_laplace, H_KSG_laplace),

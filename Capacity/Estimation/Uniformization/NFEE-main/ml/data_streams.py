...
class DataSubSampler(DataStream):
    ...
    def __init__(self, xs):
        ...
        for x in xs[1:]:
            ...
            assert (
                N == Nk
            ), "All data arrays must have the same number of elements in their first dimension."

        # set remaining class properties
        self.index_stream = IndexSubSampler(N)
        self.xs = [
            theano.shared(x.astype(theano.config.floatX), name="data" + str(i))
...
class IndexSubSampler(DataStream):
    ...
    def __init__(self, num_idx, rng=np.random):
        num_idx = int(num_idx)
        assert (
            isinstance(num_idx, int) and num_idx > 0
        ), "Number of indices must be a positive integer."

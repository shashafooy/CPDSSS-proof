...
def run_CPDSSS(sim_model,base_samples,old_thread = None,old_correction = 0,model_name = '', model_path = ''):
    ...
    if TRAIN_ONLY:
        estimator = ent.learn_model(sim_model,
                    pretrained_model=model,
                    train_samples= base_samples)
        H=0
    else:
        if KNN_THREADING:            
            new_thread,new_correction,estimator = ent.calc_entropy_thread(sim_model,base_samples = base_samples,model = model)
    ...
    if SAVE_MODEL:
        _ = ent.update_best_model(estimator.model,base_samples,name=model_name,path=model_path)
...
for i in range(n_trials):        
    ...
    for k, T in enumerate(T_range):
        ...
        if KNN_THREADING:
            H_joint[prev_idx],H_hxc_thread,H_hxc_correction = run_CPDSSS(sim_model,hxc,H_joint_thread,H_joint_correction,name,XH_path)
            MI[prev_idx] = H_hxc[prev_idx] + H_xxc[prev_idx] - H_joint[prev_idx] - H_cond[prev_idx]
        else:
            H_hxc[index]=run_CPDSSS(sim_model,hxc,model_name=name,model_path=XH_path)
        ...
        name=f'{T-1}T'
        if KNN_THREADING:
            H_hxc[index],H_cond_thread,H_cond_correction = run_CPDSSS(sim_model,X_cond,H_hxc_thread,H_hxc_correction,name,X_path)
        else:
            H_cond[index]=run_CPDSSS(sim_model,X_cond,model_name=name,model_path=X_path)
        ...
        if KNN_THREADING:
            H_cond[index],H_xxc_thread,H_xxc_correction = run_CPDSSS(sim_model,X,H_cond_thread,H_cond_correction,name,X_path)
        else:
            H_xxc[index]=run_CPDSSS(sim_model,X,model_name=name,model_path=X_path)        
        if not TRAIN_ONLY:    
            util.io.save((T_range, MI,H_hxc,H_xxc,H_joint,H_cond,i), os.path.join(path,filename)) 
        ...
        if KNN_THREADING:
            H_xxc[index],H_joint_thread,H_joint_correction = run_CPDSSS(sim_model,joint,H_xxc_thread,H_xxc_correction,name,XH_path)
        else:
            H_joint[index]=run_CPDSSS(sim_model,joint,model_name=name,model_path=XH_path)

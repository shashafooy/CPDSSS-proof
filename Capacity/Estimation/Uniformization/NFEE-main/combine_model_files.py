...
min_samples = 20000 #samples to generate per entropy calc
...
for folder in sub_folders:
    ...
    T = int(folder[:-1])
    print(f"Comparing models for {T}T")
    ...
    if folder != sub_folders[0]:
        print(f"Comparing models {T}T with previous {T-1}T")
        sim_model.set_dim_hxc()
...
#Compare with models with different T, but same distribution
for i,folder in enumerate(sub_folders[1:]):
    base_folder = os.path.join(model_path_old,folder)
    prev_folder = os.path.join(model_path_old,sub_folders[i])

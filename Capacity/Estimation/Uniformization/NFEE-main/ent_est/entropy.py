...
def mi_lnc(y, dim_x, n=None, k=1, alpha=None, shuffle=True, rng=np.random):
    ...
    mi = (
        spl.digamma(k)
        - 1 / k
        - np.mean(spl.digamma(n_x) + spl.digamma(n_y))
        + spl.digamma(N)
        - np.mean(LNC)
    )
...
def learn_density(
    ...
    if ws is None:
        ...
        trainer.train(
            minibatch=minibatch, patience=patience, monitor_every=monitor_every, logger=logger
        )

    return model


def learn_conditional_density(
    model,
    xs,
    ys,
    ws=None,
...
class UMestimator_mi:
    ...
    def learn_transformation(self, n_samples, logger=sys.stdout, rng=np.random):
        ...
        learn_density(
            self.model_j, self.samples, monitor_every=monitor_every, logger=logger, rng=rng
        )
        logger.write("training done\n")

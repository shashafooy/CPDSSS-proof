...
for filename in os.listdir(filepath):
    ...
    _stages,_H_unif_KL,_H_unif_KSG,_H_KL,_H_KSG = util.io.load(os.path.join(filepath, filename))

    # Initialize arrays
    if 'H_KL' not in locals():
        stages = _stages
    ...
    H_KL,_ = viewData.align_and_concatenate(H_KL,_H_KL,(stages),(_stages))
    H_KSG,_ = viewData.align_and_concatenate(H_KSG,_H_KSG,(stages),(_stages))
...
plt.plot(stages,mean_unif_KL,'o')
plt.plot(stages,mean_unif_KSG,'s')
plt.axhline(mean_KL)
plt.axhline(mean_KSG)
plt.yscale("log")    
...
plt.plot(stages,RMSE_unif_KSG,'bs',)
plt.axhline(RMSE_KL)
plt.axhline(RMSE_KSG)
plt.yscale("log")    

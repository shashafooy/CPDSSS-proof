...
for N=N_range
    ...
    MI_F_monteCarlo(N)=mean(MI_F)/T; %scale MI my number of symbols
    MI_G_monteCarlo(N)=mean(MI_G)/T; %scale MI my number of symbols

end
max_cap=log2(det(eye(N)+rho/M* eye(N)));
figure(1)
% plot([M:20],MI_F_monteCarlo(M:20)); hold on

plot(N_range,MI_G_monteCarlo(N_range)); hold on
...
% 
% figure(2)
% plot([M:20],MI_G_monteCarlo(M:20));
% title('Mutual Information M=N=' + num2str(M) + ', rho(dB) = ' + num2str(rho))
% xlabel('T')
% ylabel('bits/T')

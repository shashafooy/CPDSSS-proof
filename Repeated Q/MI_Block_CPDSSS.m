...
for num_block=1:BLOCKS
    ...
    s=(normrnd(0,sqrt(sigma2),N/L,iter*num_block)+1i*normrnd(0,sqrt(sigma2),N/L,iter*num_block))/sqrt(2);
    v=(normrnd(0,sqrt(sigma2_v),NNL,iter*num_block)+1i*normrnd(0,sqrt(sigma2_v),NNL,iter*num_block))/sqrt(2);

    x_tx=zeros(N,iter*num_block);
    x_no_noise = zeros(N,iter*num_block);
    ...
    for i=1:num_block:iter*num_block-num_block
...
%     H_xs_no_noise=1/2*log(det(cov_xs_no_noise))+length(cov_xs_no_noise)/2*(log(2*pi)+1);
    ...
    MI_cov_MSD(num_block)=H_MSD(sN) + H_MSD(xN) - H_MSD([xN;sN]);
